...
useEffect(() => {
  ...
  // ── 레벨 10 전용: hingePosIndex 에 따라 hingeBox/nail 위치 업데이트
  useEffect(() => {
    if (currentLevel !== 10) return;
    const world = engineRef.current.world;

    // Matter.Composite 로 body 목록 조회
          ...
          <div className="absolute top-2 left-1/2 transform -translate-x-1/2 flex gap-2 z-20">
            <button
              onClick={() => {
                  ...
                  // 1️⃣ locally update hinge index
                  setHingePosIndex(newIndex);
                  // 2️⃣ force our own world to rebuild
                  setResetTrigger(t => !t);
                  // 3️⃣ let the other client know
